<template>
  <div class="row">
    <div class="col-md-6">
      <div class="form-group" :class="{ 'has-error': errors.customer_id }">
        <label for>
          Customer
          <sup>
            <a href="javascript:void(0)" @click="newCustomer">New Customer</a>
          </sup>
        </label>
            ...
            <tr v-for="(row, index) in transactions.detail" :key="index">
              ...
              <td>
                <div class="input-group">
                  ...
                  >{{ row.laundry_price != null && row.laundry_price.unit_type == 'Kilogram' ? 'gram':'pcs' }}</span>
                </div>
              </td>
              <td>Rp {{ row.price }}</td>
    ...
    <div class="col-md-12" v-if="isSuccess">
      <div class="alert alert-success">Transaksi Berhasil, Total Tagihan: Rp {{ total }}</div>
    </div>
  </div>
</template>
